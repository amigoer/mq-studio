...
- 字段级密钥由主密钥派生
- 空字符串不加密
- `secret.key` 损坏时启动会保留原文件并报告错误，不会生成新密钥覆盖，避免现有密文永久不可恢复
- 全量导出为跨设备迁移格式，连接和全局 ACL 凭据会以明文写入导出 JSON；文件权限为 0600，前端导出前会明确二次确认
- 导入时会先校验并完成本机加密，再原子替换设置和连接文件；失败自动回滚，成功后立即热重载，无需重启应用
...
2. **本地运行**：不强制公网暴露管理端口
3. **配置导入导出**：经 `SettingsService` 原子处理并热加载；导出文件包含明文凭据，必须按敏感文件保管
