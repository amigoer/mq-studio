...
export const fetchLatestMessages = (topic: string, maxResults: number): Promise<MessageItem[]> =>
  MessageService.Query(ACTIVE_CONNECTION, { topic, key: '', tag: '', maxResults, startTime: 0, endTime: 0 }).then(
    present,
  )
...
export function queryMessagesByCondition(
  ...
  if (condition.messageId?.trim())
    return MessageService.ByID(ACTIVE_CONNECTION, topic, condition.messageId.trim()).then((item) =>
      item ? [item] : [],
    )
...
export const resendMessage = (
  ...
  topic: string,
  messageId: string,
): Promise<string> => MessageService.Resend(ACTIVE_CONNECTION, { consumerGroup, clientId, topic, messageId })
...
export const sendMessage = (
  ...
  body: string,
  delayLevel = 0,
): Promise<string> => MessageService.Send(ACTIVE_CONNECTION, { topic, tags, keys, body, delayLevel })
