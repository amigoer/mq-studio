...
export const createConsumerGroup = (
  ...
  consumeMode: string,
  maxRetry: number,
): Promise<void> => ConsumerService.Create(ACTIVE_CONNECTION, { group, brokerAddr, consumeMode, maxRetry })
export const updateConsumerGroup = (
  group: string,
  brokerAddr: string,
  consumeMode: string,
  maxRetry: number,
): Promise<void> => ConsumerService.Update(ACTIVE_CONNECTION, { group, brokerAddr, consumeMode, maxRetry })
...
export const resetOffset = (
  ...
  timestamp: number,
  force: boolean,
): Promise<void> => ConsumerService.ResetOffset(ACTIVE_CONNECTION, { group, topic, timestamp, force })
