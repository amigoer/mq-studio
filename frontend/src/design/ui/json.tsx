...
export const JStr = ({ children }: { children: ReactNode }) => (
  <span style={{ color: "var(--c-ok-text)" }}>{children}</span>
);
export const JNum = ({ children }: { children: ReactNode }) => (
  <span style={{ color: "var(--c-info-text)" }}>{children}</span>
);
export const JDim = ({ children }: { children: ReactNode }) => (
  <span style={{ color: "var(--c-muted)" }}>{children}</span>
);
        ...
        return (
          <div key={i} style={{ display: "flex", gap: "10px" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              {!last && <span style={{ width: "1px", flex: 1, background: "var(--c-border-soft)" }} />}
            </div>
            <div style={{ paddingBottom: last ? undefined : "10px", fontSize: "11.5px" }}>
              <b style={{ fontWeight: 500 }}>{step.title}</b>{" "}
              <span className="mono3" style={{ color: "var(--c-muted)", fontSize: "10.5px" }}>
                {step.meta}
              </span>
