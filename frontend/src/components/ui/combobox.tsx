...
export function Combobox({
  ...
  const { entries, hidden, sectioned } = React.useMemo(() => {
    ...
    const out: Entry[] = [];
    if (emptyLabel.toLowerCase().includes(needle))
      out.push({ value: "", label: emptyLabel });
      ...
      <Popover.Portal>
        ...
        >
          ...
          <div
            ...
            style={{
              maxHeight:
                "min(280px, var(--radix-popover-content-available-height))",
            }}
            onKeyDown={(e) => {
          ...
          >
            {entries.length === 0 && emptyMessage && (
              <div className="text-muted-foreground px-2 py-2 text-fs-115">
                {emptyMessage}
              </div>
            )}
            {entries.map((entry, i) => {
              ...
              const heading =
                sectioned &&
                entry.section &&
                entry.section !== entries[i - 1]?.section
