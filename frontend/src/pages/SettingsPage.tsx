...
const GITHUB_RELEASES_URL =
  "https://github.com/amigoer/mq-studio/releases/latest";

type SectionId =
  "appearance" | "general" | "fonts" | "message" | "proxy" | "data" | "about";
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1 pr-2">
        ...
        {hint && (
          <div className="text-muted-foreground mt-1 text-fs-12">{hint}</div>
        )}
      </div>
          ...
          return (
            ...
            >
              ...
              >
                <div>
                  <div
                    className="text-fs-13 font-medium"
      ...
      <Card>
        ...
        >
          <Switch
            ...
            onCheckedChange={() => setAnimations(!prefs.animations)}
          />
        </SettingsRow>
      </Card>
    </>
      ...
      <Card>
        ...
        >
          <Switch
            ...
            onCheckedChange={() =>
              setSetting("autoConnectLast", !settings.autoConnectLast)
            }
          />
        </SettingsRow>
        <SettingsRow
        ...
        >
          <Switch
            ...
            onCheckedChange={() =>
              setSetting("autoCheckUpdate", !settings.autoCheckUpdate)
            }
          />
        </SettingsRow>
      </Card>
      ...
      <Card>
        ...
        >
          <Select
            ...
            onChange={(e) =>
              setSetting("closeBehavior", e.target.value as CloseBehavior)
            }
          >
    ...
    <>
      ...
      <Card>
        ...
        >
          <Select
            ...
            onChange={(e) =>
              setSetting("timestampFormat", e.target.value as TimestampFormat)
            }
          >
      ...
      <Card style={{ padding: 16 }}>
        ...
        <div
          ...
          style={{
            fontFamily: `"${settings.monospaceFont}", ui-monospace, monospace`,
          }}
        >
      ...
      <Card>
        ...
        >
          <Select
            ...
            onChange={(e) =>
              setSetting("fetchLimit", Number(e.target.value) as FetchLimit)
            }
          >
            {FETCH_LIMITS.map((n) => (
        ...
        >
          <Switch
            ...
            onCheckedChange={() =>
              setSetting("autoFormatJson", !settings.autoFormatJson)
            }
          />
        </SettingsRow>
        <SettingsRow
          ...
          <Input
            ...
            onChange={(e) =>
              setSetting(
                ...
                (Number(e.target.value) || 500) * 1024,
              )
            }
            onBlur={() => {
              const kb = Math.max(
                ...
                Math.min(
                  4096,
                  Math.round(settings.maxPayloadRenderBytes / 1024),
                ),
              );
      ...
      <Card>
        ...
        >
          <Input
            ...
            onChange={(e) =>
              setSetting("lagAlertThreshold", Number(e.target.value) || 0)
            }
          />
          <span className="text-muted-foreground text-fs-12">
            {t("settings.message.lagAlertUnit")}
          </span>
        </SettingsRow>
        <SettingsRow
        ...
        >
          ...
          />
          <span className="text-muted-foreground text-fs-12">
            {t("settings.message.diskAlertUnit")}
          </span>
        </SettingsRow>
        <SettingsRow
...
function ProxyPanel() {
  ...
  const [credentialsBusy, setCredentialsBusy] = useState<
    "save" | "clear" | null
  >(null);
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("settings.proxy.credentialsSaveError"),
      );
    } finally {
  ...
  const handleClearCredentials = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("settings.proxy.credentialsClearError"),
      );
    } finally {
      setCredentialsBusy(null);
    }
  ...
  return (
    <>
      ...
      <Card>
        <SettingsRow
          title={t("settings.proxy.connect")}
          hint={t("settings.proxy.connectHint")}
        >
          <Input
            type="number"
            style={{ width: "7.69rem" }}
            ...
            onChange={(e) =>
              setSetting("connectTimeoutMs", Number(e.target.value) || 3000)
            }
            onBlur={() =>
              setSetting(
                "connectTimeoutMs",
        ...
        >
          <Input
            ...
            onChange={(e) =>
              setSetting("requestTimeoutMs", Number(e.target.value) || 5000)
            }
            onBlur={() =>
              setSetting(
                "requestTimeoutMs",
      ...
      <Card>
        <SettingsRow
          title={t("settings.proxy.ak")}
          hint={t("settings.proxy.akHint")}
        >
          <Input
            type="text"
        ...
        <div className="flex justify-end gap-2 px-4 py-3">
          ...
          <Button
            ...
            size="sm"
            disabled={
              credentialsBusy != null || !accessKey.trim() || !secretKey.trim()
            }
      ...
      <Card>
        <div
          className="text-muted-foreground text-fs-12"
          style={{ padding: "12px 16px", lineHeight: 1.55 }}
        >
        ...
        {DATA_PATHS.map((p, i) => {
          ...
          return (
            <div
              ...
              className={cn(
                "flex cursor-pointer items-center gap-3 hover:bg-muted/40",
                ROW_FOCUS_CLASS,
              )}
              style={{
                padding: "10px 16px",
      ...
      <Card style={{ padding: 18 }}>
        <div className="flex items-start gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p
              className="text-muted-foreground text-fs-115"
...
export function SettingsPage() {
  ...
  const { resetAllSettings, reloadSettings, settlePendingSaves, loading } =
    useSettings();
  const { refresh: refreshConnections } = useConnections();
  const { available: updateAvailable, refresh: refreshUpdate } =
    useUpdateCheck();
  ...
  const handleCheckUpdate = useCallback(async () => {
    try {
      ...
      if (result.status === "available") {
        ...
        toast.info(
          t("settings.about.upToDate", { version: result.currentVersion }),
        );
      }
    } catch {
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <PageBody width="content" className="bg-background">
          <PageTransition transitionKey={activeSection} variant="panel">
