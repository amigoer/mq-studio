...
export function AclPage({ onNavigate }: { onNavigate?: (id: NavId) => void }) {
  ...
  const activeKey = activeConn
    ? `${activeConn.id}:${activeConn.endpoints}`
    : "";
  ...
  const [defaultTopicPerm, setDefaultTopicPerm] =
    useState<(typeof PERMS)[number]>("DENY");
  const [defaultGroupPerm, setDefaultGroupPerm] =
    useState<(typeof PERMS)[number]>("SUB");
  ...
  const overrideCount =
    parsePermLines(topicPerms).length + parsePermLines(groupPerms).length;
            ...
            <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
              <div className="flex flex-col gap-5">
                ...
                <Card style={{ padding: 20 }}>
                  <div className="mb-4">
                    <div className="text-fs-13 font-semibold">
                      {t("acl.form.title")}
                    </div>
                    <div className="text-muted-foreground mt-0.5 text-fs-12">
                      {t("acl.form.subtitle")}
                    </div>
                  </div>
                  <div
                      ...
                      <>
                        <div>
                          ...
                          <Select
                            ...
                            onChange={(e) =>
                              setDefaultTopicPerm(
                                e.target.value as (typeof PERMS)[number],
                              )
                            }
                          >
                            {PERMS.map((p) => (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      style={{ color: "hsl(var(--destructive))" }}
                      onClick={() =>
                        deleteAk.trim() && setConfirmDelete(deleteAk.trim())
                      }
                  ...
                  >
                    ...
                    <div className="flex justify-end">
                      ...
                      >
                        ...
                        {whiteSaving
                          ? t("acl.globalWhite.saving")
                          : t("acl.globalWhite.save")}
                      </Button>
                    </div>
                  </div>
                </Card>
