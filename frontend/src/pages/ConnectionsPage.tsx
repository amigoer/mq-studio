    ...
    nsEntries = [
      {
        ...
        port: prefill.port?.trim() || DEFAULT_NS_PORT,
      },
    ];
  }
  return {
...
export function ConnectionsPage() {
  ...
  useEffect(() => {
    ...
    if (
      hydrated &&
      hydrated.id === selected.id &&
      hydrated.token === resyncToken
    )
      return;
    ...
    setAdvancedOpen(
      hasACL(selected) || selected.timeoutSec !== 5 || !!selected.remark,
    );
    ...
    if (f.enableACL) {
      ...
      if (
        ...
        (!replacingCredentials &&
          (!f.accessKeyConfigured || !f.secretKeyConfigured))
      ) {
  ...
  const persistForm = async (opts?: {
    ...
    if (isNew) {
      ...
      if (!opts?.quiet) {
        toast.success(
          t("connections.createSuccess", { name: form.name.trim() }),
        );
      }
      await refresh();
  ...
  const handleConnectPrimary = async () => {
    ...
    try {
      ...
      const conn = (await connectionApi.getConnections()).find(
        (c) => c && c.id === id,
      );
      ...
      toast.success(
        t("connections.connectSuccess", {
          name: form.name.trim() || conn?.name || String(id),
        }),
      );
      await refresh();
      setSelectedId(id);
  ...
  const handleDisconnectPrimary = async () => {
    ...
    try {
      ...
      toast.success(
        t("connections.disconnectSuccess", { name: selected.name }),
      );
      await refresh();
    } catch (e) {
      toast.error(formatErrorMessage(e));
  ...
  const handleSetDefault = async () => {
    ...
    try {
      ...
      toast.success(
        t("connections.setDefaultSuccess", { name: selected.name }),
      );
      await refresh();
    } catch (e) {
      toast.error(formatErrorMessage(e));
  ...
  const primaryBusy =
    busy === "connect" || busy === "disconnect" || busy === "save";
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={anyBusy}
            title={
              online ? t("connections.disconnect") : t("connections.connect")
            }
            onClick={(e) =>
            ...
            {loading && list.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {groups.map((g) => {
                  ...
                  const collapsed =
                    !search.trim() && collapsedGroups.has(g.name);
                  return (
                    <div
                      key={g.name || "__ungrouped__"}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label={t("connections.group")}>
                    <Combobox
                      ...
                      searchPlaceholder={t(
                        "connections.groupSearchPlaceholder",
                      )}
                      createLabel={(name) =>
                        t("connections.groupCreate", { name })
                      }
                            ...
                            {form.nsEntries.length > 1 && (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setForm({
                                    ...form,
                                    nsEntries: form.nsEntries.filter(
                                      (_, i) => i !== index,
                                    ),
                                  });
                                }}
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              timeoutSec: Number(e.target.value) || 1,
                            })
                          }
                        />
