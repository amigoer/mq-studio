...
func TestReloadSerializesWithConcurrentConnect(t *testing.T) {
	...
	connection, err := service.AddConnection("primary", string(model.EnvTest), "primary:9876", 5, false, "", "", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestReplaceConnectionsSerializesWithConcurrentConnect(t *testing.T) {
	service := newTestService(t, nil)
	connection, err := service.AddConnection("old", string(model.EnvTest), "old:9876", 5, false, "", "", "")
	if err != nil {
		t.Fatal(err)
	}
	runtime := newBlockingRuntime()
